...
void Model::draw(std::shared_ptr<ShaderProgram> shader) {
    for (const auto& mesh : meshes) {
        if (mesh->material) {
            ...
            if (material->tDiffuse)   material->tDiffuse->use(0);
            if (material->tSpecular)  material->tSpecular->use(1);
            if (material->tNormal)    material->tNormal->use(2);
...
std::shared_ptr<Model> Model::load_from_file(const std::filesystem::path& path) {
    ...
    for (const auto& shape : o_shapes) {
        ...
        mesh->vao.bind();
        mesh->vbo.set_data(buffer, GL_STATIC_DRAW);
        mesh->ebo.set_data(indices, GL_STATIC_DRAW);
